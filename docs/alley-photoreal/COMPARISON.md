...
当前运行时已从 512 px 包切换到本地 ORCA 安全版本压制的 768 px 包；页面运行时会写入 `data-alley-texture-pbr="basecolor-normal-shader-wetness"`，说明该包确实提供了颜色和法线，但没有嵌入 roughness/AO 通道。墙面和石板的 shader 现在会补上高频微法线、干湿粗糙度和积水反射，但这仍不是完整 ORM/高度贴图。下一轮最有价值的工作是补齐默认机位内可见墙面与石板的独立 roughness/AO/height 通道，并恢复受简化损伤的窗框、石材边缘。
